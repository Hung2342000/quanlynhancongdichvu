...
<script type="text/javascript">
  ...
  function togglePassword(event) {
      ...
      const showPasswordLink = event.currentTarget;

      // Kiểm tra trạng thái hiện tại của ô mật khẩu
      const isPasswordVisible = passwordField.type === 'text';

      // Chuyển đổi giữa 'text' và 'password' và cập nhật data-show-pass
      passwordField.type = isPasswordVisible ? 'password' : 'text';
      showPasswordLink.setAttribute('data-show-pass', isPasswordVisible ? 'password' : 'text');

      // Thay đổi icon khi mật khẩu đang hiển thị hoặc bị ẩn
      showPasswordLink.innerHTML = isPasswordVisible ? '<i class="fas fa-eye"></i>' : '<i class="fas fa-eye-slash"></i>';
  }
  <?php }?>
</script>
            ...
            <div class="pt-10 pb-6 pl-11 pr-12 bg-black-2 h-100 d-flex flex-column dark-mode-texts">
                <div class="pb-9">
                    <h3 class="font-size-7 text-white line-height-reset pb-4 line-height-1p4">
                        <img style="max-width: 40%;" src="<?=$logoFileUrl?>"/>
                        <h3>Quản lý nhân công dịch vụ</h3>
                    </h3>
                    ...
                    <div class="bg-white-2 h-100 px-11 pt-11 pb-7">
                        ...
                        <form id="loginForm" action="login.php" method="POST">
                            ...
                            <div class="form-group">
                                <label for="username" class="font-size-4 text-black-2 font-weight-semibold line-height-reset"> Tên đăng nhập</label>
                                <input class="form-control" placeholder="Nhập tên đăng nhập" id="username" name="username">
                            </div>
                            <div class="form-group">
                                <label for="password" class="font-size-4 text-black-2 font-weight-semibold line-height-reset">Mật khẩu</label>
                                <div class="position-relative">
                                    <input type="password" id="password" name="password" class="form-control" id="password" placeholder="Nhập mật khẩu">
                                    <a href="#" class="show-password pos-abs-cr fas mr-6 text-black-2" data-show-pass="password" onclick="togglePassword(event)">
                                    </a>
                                </div>
                            </div>
                            <?php if (isset($_REQUEST['f'])) {?>
                                <div class="alert alert-danger" role="alert">
                                    <i class="fa fa-theater-masks" style="padding-right: 10px;"></i> Đăng nhập thất bại
...
<!--                                </label>-->
                            ...
                            <div class="form-group mb-8">
                                <button class="btn btn-info btn-medium w-100 rounded-5 text-uppercase" type="button" onclick="submitLogin();return false;">Đăng nhập </button>
                            </div>
<!--                            <p class="font-size-4 text-center heading-default-color">Bạn quên mật khẩu? vui lòng liên hệ quản trị viên để lấy lại <a href="" class="text-info" onclick="showForgotPassword();return false;">Reset Password</a></p>-->
                            <p class="font-size-4 text-center heading-default-color">Bạn quên mật khẩu? vui lòng liên hệ quản trị viên để lấy lại</p>
                        </form>
                        <form id="requestPasswordChangeForm" style="display:none;" action="">
                            <div class="form-group">
                                <label for="username" class="font-size-4 text-black-2 font-weight-semibold line-height-reset">Tên đăng nhập</label>
                                <input class="form-control"  id="usernameChange" name="usernameChange">
                            </div>
